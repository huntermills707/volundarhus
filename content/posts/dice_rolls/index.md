...
The UI instantly updates two graphs: a *probability mass function* (individual outcomes) and a *cumulative distribution* (chance of meeting or exceeding a threshold). The mass plot also displays the four statistical moments (mean, standard deviation, skewness, kurtosis), so you can see at a glance how "tight" or "wild" a method is.
...
One thing the app makes obvious is how larger dice pools converge on a normal (Gaussian) distribution. If you roll, say, 10d6 and drop the two lowest, the resulting histogram looks almost perfectly bell-shaped.
Statistic (Ex: 10d6).
...
Not every table wants to juggle a handful of dice. The app makes it easy to compare a **complex method** (many dice) against a **simpler equivalent** (fewer dice plus a static modifier).
...
Dice don't have to be a mystery. A small Plotly Dash app, a few lines of Python, is enough to see what a house rule actually does to the distribution before you adopt it at the table.

The source is available [here](https://github.com/huntermills707/DandDandD_StatRolls) with very few dependencies. Let me know what you think, or if other ideas should be added. Happy rolling!