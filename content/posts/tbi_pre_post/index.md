...
Traumatic brain injury is already recognized as a leading cause of disability worldwide. Yet most large-scale epidemiologic work has either (a) excluded patients with pre-existing psychiatric diagnoses or (b) lacked a non-head trauma comparator. Without one, it is hard to tell whether the observed mental-health sequelae come from the brain injury itself or from the broader trauma experience: hospitalization, pain, loss of function, and so on.

We fill that gap in three ways:
...
3. **Tracking both pre-index (-1 year) and post-index (up to +7 years)** outcomes, giving a view of baseline risk and the incremental effect of the injury.
...
2. **Early post-injury window is critical.** The 6-12-month surge in suicidality aligns with the period when patients transition from acute care to community living, often hitting gaps in follow-up. Targeted screening and intervention during this window could avert many adverse outcomes.
3. **Screening should be universal.** The study deliberately included patients with prior psychiatric histories, so the findings reinforce that all TBI survivors need systematic mental-health assessment, not just the "clean" cases.
4. **Risk stratification by age and sex.** Both males and females showed heightened risk, but certain age brackets (young adults for suicidality; middle-aged and older adults for PTSD) may benefit from tailored monitoring.
5. **Policy and health-system actions.** Automated EHR alerts (e.g., flagging a TBI diagnosis to trigger a mental-health consult) could operationalize the recommendation for uniform screening.
...
* Routine, automated mental-health screening built into trauma pathways could prevent a lot of this morbidity.

Future work should parse out the contribution of injury severity, look for neuroimaging or biomarker correlates of the mental-health risk, and test targeted preventive programs in randomized trials.
