...
Our recent study shows that a single-lead deep-learning model can turn that ordinary ECG into a useful PH screener. It picks up overt disease and clinically important subtypes, and it does so up to two years before conventional diagnosis.
...
* Using only two leads (I + V2) or a single lead (V2) reduces AUC modestly (≈0.83-0.84) but still yields clinically useful performance, which opens the door for wearable or remote monitors.
...
## Bottom line
A deep-learning CNN can extract hidden PH signals from a standard 12-lead ECG with AUC≈0.9, even when only a single lead is available. Because the ECG is already ubiquitous, this could bring PH screening to the bedside, and maybe even the wrist, and shrink the lag between symptom onset and definitive diagnosis.
