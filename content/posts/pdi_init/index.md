...
Most existing socioeconomic measures (ADI, NDI, SVI) rely on PCA-style factor analysis. That approach forces you to fill every missing cell with a crude average and produces “centered” scores that hide the real magnitude of deprivation. The specific problems:

* **Missing data gets mean-imputed**, which distorts local realities. It also assumes data is missing at random, which is often false. Rural areas have greater missingness.
...
|Constraint|Effect|
|---|---|
|Non-negativity on the first latent dimension ($X_1 \ge 0$, $Y_1 \ge 0$)|Guarantees the primary component is a pure accumulation of deprivation signals, so there is no “negative poverty”.|
...
Feel free to open issues, submit pull requests, or fork the repo and adapt it to your own data.
