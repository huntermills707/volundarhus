...
Since then, I've expanded the simulator to handle full stat generation workflows, including modification rules like dropping or replacing specific stats.

If you're a Game Master trying to balance a campaign, a player wondering if your "heroic" house rule is actually too generous, or a developer curious about exact probability calculations without Monte Carlo sampling, this post is for you.


The app is live on [PyCafe](https://py.cafe/app/huntermills707/dash-dice-roll-probability) and the source is on [GitHub](https://github.com/huntermills707/DandDandD_StatRolls).

EDIT 3/12: In case PyCafe service is down. Link to Render.com Hobby Tier Deployment: [here](https://danddandd-statrolls.onrender.com).
Slow performance on this deployment compared to PyCafe.
...
Why it matters:
Most online calculators stop at the single-roll distribution. But in TTRPGs, you care about the *set* of stats. How likely is a "god-tier" array (three 16+)? What's the chance of ending up with two stats below 8? The app answers these by enumerating all possible combinations of stats rather than individual rolls.
...
TTRPG implications:
- **Elite NPCs**: Give a boss monster a "blessed" die pool to keep them consistently above average.
...
Use case:
A "hybrid" system where you roll 4d6 drop lowest for 6 stats, but any stat below 8 is automatically raised to 8. This prevents campaign-breaking weak characters while keeping the randomness.
...
With standard 4d6 drop lowest, there's still a ~15% chance a character ends up with two stats ≤ 8. In a party of 4, that's a 40% chance someone rolls a "liability." Great for some groups, not so great for others.
...
Some groups adopt "heroic" rules like 5d6 drop 2, thinking it only raises the mean. It also reduces variance, so every character ends up feeling similarly powerful.

What the app reveals:
- The skewness and kurtosis metrics show how the distribution shape changes.
- Lower kurtosis means fewer outliers, weak and strong alike, which can flatten the game's drama.
...
The key insight: instead of simulating thousands of rolls, enumerate every possible outcome.
...
Key detail: the function sorts stats first, then slices or replaces. This keeps the behavior deterministic.

---

### 3. Combinatorial Enumeration for Stat Sets (`stats.py`)

Generating *z* stats isn't the same as repeating the roll *z* times. We need the joint distribution of all stats.

The app uses `combinations_with_replacement` to enumerate stat sets efficiently:
...
These metrics let you assess the distribution's shape quickly, without staring at graphs.
...
- **Enhance Visualizations**: Allow multiple configurations to be plotted simultaneously.
...
Dice are design tools as much as randomizers. Quantifying each house rule lets you prevent campaign-breaking imbalances and tune character power to your campaign's tone.
