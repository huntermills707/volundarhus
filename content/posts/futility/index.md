...
In the Intensive Care Unit (ICU), "medical futility" is a lightning rod for ethical conflict. Clinicians often estimate that 20–40% of patients receive treatments they perceive as non-beneficial. Yet despite how common these conflicts are, the word "futility" itself almost never appears in the medical record.
...
2. **Semantic Shift:** Clinicians aren't ignoring the concept; they are describing it using different vocabulary. They talk about *prognosis*, *risks*, *code status*, and *sentiments* rather than labeling a treatment "futile."

## Eight Themes of "Futility"

We didn't stop at counting words; we mapped the vocabulary around them. Using word2vec models trained annually on the entire ICU corpus, we identified the terms most statistically associated with "futile/futility" and grouped them into eight distinct themes:
...
The data show a clear shift in how these concepts are recorded. While the overall frequency of the word "futility" remained flat, the *context* changed.
...
If we train AI on medical notes expecting it to identify "futile care" based on the keyword "futile," we will miss 99.9% of the relevant cases. Our study shows that the semantic context of futility is heterogeneous and evolving.
...
* **Explainability:** For LLMs to be trusted in critical care, they must understand that "futility" is often a proxy for a complex negotiation of autonomy, prognosis, and family dynamics, rather than a binary physiological failure.

## Conclusion

Medical futility is not a static definition; it is a dynamic conversation. Ten years of raw clinical text show clinicians navigating this with nuance, usually avoiding the loaded term "futility" in favor of documenting the *process* of care and the *status* of the patient.

For the next generation of health AI, the lesson is simple: don't look for the word. Look for the pattern.
