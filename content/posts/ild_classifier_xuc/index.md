...
Identifying patients with interstitial lung disease (ILD) in large electronic health record (EHR) repositories has always been a nightmare. Diagnostic codes are noisy. Rule-based filters either miss too many true cases (low sensitivity) or pull in a pile of false positives (low PPV). It is common to build a "Gold Standard" data set using these rules in rare disease prediction, even though the label quality is low. Some researchers achieve high metrics with these poor labels, but it begs the question: what is a model worth if it matches substandard labels with high accuracy?

Our team set out to replace that brittle logic with a machine-learning model that learns the patterns in routine clinical data against high-quality labels, and, crucially, works across health systems without custom rewrites.
...
* Sensitivity jumps while PPV stays competitive.
* Misclassification rates drop hard (McNemar’s test p<10e-13 vs. Approach 1, p = 0.004 vs. Approach 2).
* Error patterns differ too (Kolmogorov–Smirnov test p<2.2e-16). The ML model isn’t just shifting mistakes around; it is genuinely better at separating signal from noise.
...
Our Universal ILD Classifier beats the rule-based hacks on every metric that matters, works across three separate health systems with no site-specific tweaking, and opens the door to genuinely large-scale, reproducible ILD research. The model isn’t perfect (structured data alone limits its ceiling), but it is a big step up from “one code equals disease.”

If you’re building phenotyping pipelines, stop polishing ICD-code trees and start feeding your data into a gradient-boosted, OMOP-compatible model. The gains are real, and for rare-disease research they add up fast.
