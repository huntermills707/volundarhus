...
math: true
description: "A Python module that brings Stata-style marginal effects to StatsModels: a unified delta-method framework with numerical Jacobians for adjusted predictions, marginal effects, and discrete contrasts on any GLM-family model, with patsy handling polynomials, interactions, and splines correctly. Includes a difference-in-differences helper for nonlinear models. Roughly 500 lines."
---
...
So I wrote a small module, [`marginal_effects.py`](#), that fills in the gap. This post is a tour of what it does, the math behind it, and the design decisions that kept it small.
...
Richard Williams' [Margins01 notes](https://academicweb.nd.edu/~rwilliam/stats/Margins01.pdf) lay out the useful catalogue, and they all fit a single schema. Each is a statistic of $\beta$ that happens to also depend on the data:
...
Seven names, one unifying form. Every one of these is $g(\beta)$ for some $g$. That turns out to be the whole game.
